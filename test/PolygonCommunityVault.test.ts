...
describe("Contract Tests", function () {
  it("Should fail if no owner tries to set allowance", async function () {
    ...
    await expect(owner.Vault.setAllowance(owner.address, "1")).to.be.revertedWith(
...
describe("Events", () => {
  it("setAllowance works for owner and emits SetAllowance", async function () {
    ...
    expect(Vault.setAllowance(owner.address, "1000000000000000000000"))
      .to.emit(Vault.address, "SetAllowance");
  });
});
